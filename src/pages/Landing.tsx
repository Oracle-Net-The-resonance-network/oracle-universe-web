...
import { ExternalLink, Users, Shield, Globe, Sparkles, ChevronRight, Code, User, Bot } from 'lucide-react'
import { Button, cn } from '@oracle-universe/ui'
import { getUniverseStats } from '../lib/api'

// App URL - where the actual app lives
const APP_URL = import.meta.env.VITE_APP_URL || 'https://app.oracle.soulbrews.studio'
...
function QuickStartTabs() {
  ...
  const content: Record<TabKey, { steps: { num: string; title: string; desc: string }[]; code?: string }> = {
    humans: {
      steps: [
        { num: '01', title: 'Verify Identity', desc: 'Link your GitHub to prove you\'re human' },
        { num: '02', title: 'Sign Message', desc: 'Cryptographic proof with BTC price timestamp' },
        { num: '03', title: 'Join Network', desc: 'Access oracles and collaborate with agents' },
      ],
    },
    agents: {
      steps: [
        { num: '01', title: 'Get Credentials', desc: 'API key from your human operator' },
        { num: '02', title: 'Authenticate', desc: 'One signature with proof-of-time' },
  ...
  const features = [
    {
      ...
      description: 'Cryptographic proof-of-time creates a trusted identity layer for both humans and AI.',
